...
export default ({history}) => {
    ...
    const backAction = () =>{
      ...
      const onPress1 =()=>{history.push('/Recorder')}
      const onPress2 =()=>{console.log('clicked')}
      return(
        <View style={styles.background}>
            <StatusBar backgroundColor="black" />
            <View style={styles.settingbar}>
                <TouchableWithoutFeedback onPress={() => history.push('/Record')}>
                    <Image source={require('../../assets/right.png')} style={styles.imageRight}/>
                </TouchableWithoutFeedback>
                <Text style={styles.header}>Voice Recorder</Text>
                <Text style={styles.list} onPress={() => history.push('/List')}>List</Text>
            </View>
            <View style={styles.settingbar1}> 
                <TouchableHighlight style={styles.buttonL} underlayColor= {opablack} onPress={onPress1}>
                    <Text style={styles.textL}>Standard</Text>
                </TouchableHighlight>
                <TouchableHighlight style={styles.buttonR} underlayColor={opaColor} onPress={onPress2}>
            ...
            <View style={styles.background3}>
                <Text style={{padding:5,borderRadius:5, backgroundColor:chooseColor,color:'black',fontFamily:"Inter_500Medium",fontSize:16,top:-10,left:'5%'}}>{raw}</Text>
                {play?(<Text style={styles.text2}>PUPPUB MASK for your easier new normal</Text>):(
                <Text style={styles.text}>Speaking something</Text>)
                }
            </View>
            <View style={{padding:0,alignItems:'center',justifyContent:'center',bottom:0.07*screenHeight,position:'absolute'}}>
            {play?( 
                    <View>
                    ...
                    <View style={{flexDirection:'row',justifyContent:'space-around',width:screenWidht, height:buttonHeight/4,top:20}}>
                        ...
                        Alert.alert("Save recording","My record #1",[
                                ...
                                {text:"OK",onPress:()=>{setLan({...lan,state:1}),history.push('/List',{language:lan})}}
                                ])}}>
                ...
                :(
                    <View style={{alignItems:'center',justifyContent:'center'}}> 
                        ...
                        <View style={{top:20}}>
                        <TouchableOpacity style={styles.button} activeOpacity={0.7} onPress={()=>{setPlay(1)}}>
                        <Image source={require('../../assets/play.png')} style={{resizeMode:'contain',flex:0.8}}/>
                        </TouchableOpacity>
                        </View>
                    </View>
                )}
...
const styles = StyleSheet.create({
    ...
    background3:{
        top:200 + 0.4*screenHeight/2,
        width:screenWidht,
        borderRadius:20,
        backgroundColor:'#1A1B1C',
        alignItems: 'flex-start',
        justifyContent:'flex-start',
        height:0.55*screenHeight/2,
        position:'absolute'
    },
    header:{
    ...
    middleLine:{
        ...
        height:0.20*screenHeight-8,
        borderTopWidth:(0.20*screenHeight-6)/2 -3,
        borderBottomWidth:(0.20*screenHeight-6)/2 -3,
        borderColor:'#1A1B1C',
        width:screenWidht,
        alignItems:'center',
        justifyContent:'center'
